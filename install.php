...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (ipost('action') === 'step2') {
            ...
            try {
                ...
                if (!$dbFound) {
                    ...
                    $pdo->exec("INSERT IGNORE INTO `settings` (`skey`, `svalue`) VALUES
                        ('app_name','کارت ویزیت من'), ('allow_registration','1'), ('footer_text',''),
                        ('base_url','" . detect_base_url() . "'), ('code_length','6')");
                    $_SESSION['ins_db'] = $db;
                    $_SESSION['ins_step'] = 3;
        ...
        $db = $_SESSION['ins_db'] ?? null;
        if (!$db) { header('Location: install.php?step=2'); exit; }
        $appName = trim(ipost('app_name')) !== '' ? trim(ipost('app_name')) : 'کارت ویزیت من';
...
<div class="wrap">
  ...
  <div class="card">
    ...
    <form method="post">
      ...
      <input type="text" name="app_name" value="<?= ie(ipost('app_name') !== '' ? ipost('app_name') : 'کارت ویزیت من') ?>">
